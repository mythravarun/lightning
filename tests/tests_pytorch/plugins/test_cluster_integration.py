...
@pytest.mark.parametrize(
    "trainer_kwargs",
    [
        dict(strategy="ddp", accelerator="cuda", devices=[1, 2]),
        dict(strategy="ddp_sharded", accelerator="cuda", devices=[1, 2]),
        dict(strategy="ddp_spawn", accelerator="cpu", devices=2),
        dict(strategy="ddp_spawn", accelerator="cuda", devices=[1, 2]),
    ],
)
